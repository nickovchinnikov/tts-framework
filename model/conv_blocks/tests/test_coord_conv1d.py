...
class TestCoordConv1d(unittest.TestCase):
    def test_simple_case(self):
        ...
        rank1_model = CoordConv1d(in_channels=2, out_channels=8, kernel_size=3,
    ...
    def test_with_r(self):
        ...
        model = CoordConv1d(in_channels=2, out_channels=8, kernel_size=3, 
                            stride=1, padding=0, dilation=1, groups=1, bias=True, with_r=True)
        x = torch.randn(1, 2, 10)
        output = model(x)
        self.assertEqual(list(output.shape), [1, 8, 8])
