...
dataset = datasets.LIBRITTS(root=root, download=download)
...
class MemoryMappedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        mmapped_array = np.load(self.file_path, mmap_mode="r")

        # Yield the samples one by one
        for sample in mmapped_array:
...
# Create the dataset
dataset = MemoryMappedDataset("../checkpoints/libri_preprocessed_data.pt")
