...
class TacotronAbstract(ABC, nn.Module):
    ...
    def _init_coarse_decoder(self):
        ...
        self.coarse_decoder.r_init = self.ddc_r # type: ignore
        self.coarse_decoder.set_r(self.ddc_r) # type: ignore
    ...
    def _backward_pass(self, mel_specs, encoder_outputs, mask):
        ...
        decoder_outputs_b, alignments_b, _ = self.decoder_backward(
            ...
            self.speaker_embeddings_projected) # type: ignore
    ...
    def _coarse_decoder_pass(self, mel_specs, encoder_outputs, alignments,
        ...
        T = mel_specs.shape[1]
        if T % self.coarse_decoder.r > 0: # type: ignore
            padding_size = self.coarse_decoder.r - (T % self.coarse_decoder.r) # type: ignore
        ...
        decoder_outputs_backward, alignments_backward, _ = self.coarse_decoder(
            encoder_outputs.detach(), mel_specs, input_mask) # type: ignore
    ...
    def compute_gst(self, inputs, style_input, speaker_embedding=None):
        ...
        if isinstance(style_input, dict):
            ...
            _GST = torch.tanh(self.gst_layer.style_token_layer.style_tokens) # type: ignore
            gst_outputs = torch.zeros(1, 1, self.gst_embedding_dim).to(device)
            for k_token, v_amplifier in style_input.items():
                key = _GST[int(k_token)].unsqueeze(0).expand(1, -1, -1)
                gst_outputs_att = self.gst_layer.style_token_layer.attention(query, key) # type: ignore
                gst_outputs = gst_outputs + gst_outputs_att * v_amplifier
        elif style_input is None:
            gst_outputs = torch.zeros(1, 1, self.gst_embedding_dim).to(device)
        else:
            gst_outputs = self.gst_layer(style_input, speaker_embedding) # type: ignore
