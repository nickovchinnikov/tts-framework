...
from .layers.tacotron2 import Decoder, Encoder, Postnet
from .tacotron_abstract import TacotronAbstract
...
class Tacotron2(TacotronAbstract):
    ...
    def __init__(self,
        ...
        if self.num_speakers > 1:
            self.decoder_in_features += speaker_embedding_dim # add speaker embedding dim # type: ignore
        ...
        self.decoder = Decoder(self.decoder_in_features, self.decoder_output_dim, r, attn_type, attn_win,
                               attn_norm, prenet_type, prenet_dropout,
                               forward_attn, trans_agent, forward_attn_mask,
                               location_attn, attn_K, separate_stopnet) # type: ignore
        ...
        if self.gst:
            self.gst_layer = GST(num_mel=80,
                                 ...
                                 speaker_embedding_dim=speaker_embedding_dim if self.embeddings_per_sample and self.gst_use_speaker_embedding else None) # type: ignore
    ...
    def forward(self, text, text_lengths, mel_specs=None, mel_lengths=None, speaker_ids=None, speaker_embeddings=None):
        ...
        if self.num_speakers > 1:
            if not self.embeddings_per_sample:
                ...
                # B x 1 x speaker_embed_dim
                speaker_embeddings = torch.unsqueeze(speaker_embeddings, 1) # type: ignore
        ...
        if mel_lengths is not None:
            decoder_outputs = decoder_outputs * output_mask.unsqueeze(1).expand_as(decoder_outputs) # type: ignore
        # B x mel_dim x T_out
