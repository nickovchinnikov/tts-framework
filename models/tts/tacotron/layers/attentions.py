...
class LocationLayer(nn.Module):
    r"""Layers for Location Sensitive Attention
    ...
    def forward(self, attention_cat: Tensor) -> Tensor:
        r"""Shapes:
        attention_cat: [B, 2, C]
        """
...
class GravesAttention(nn.Module):
    ...
    def __init__(self, query_dim: int, K: int):
        super().__init__()
    ...
    def init_states(self, inputs: Tensor):
        if self.J.nelement() == 0 or inputs.shape[1] + 1 > self.J.shape[-1]:
    ...
    def forward(self, query: Tensor, inputs: Tensor, processed_inputs: Tensor, mask: Tensor):
        """Shapes:
