...
class Linear(nn.Module):
    r"""Linear layer with a specific initialization.
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool=True, init_gain: str="linear"):
        super().__init__()
        self.linear_layer = torch.nn.Linear(in_features, out_features, bias=bias)
        self._init_w(init_gain)

    def _init_w(self, init_gain: str) -> None:
        r"""Initialize weights of the linear layer.

        Args:
            init_gain (str): method to compute the gain in the weight initialization.
        """
        torch.nn.init.xavier_uniform_(self.linear_layer.weight, gain=torch.nn.init.calculate_gain(init_gain))

    def forward(self, x: Tensor) -> Tensor:
        r"""Forward pass of the Linear layer.
...
class LinearBN(nn.Module):
    r"""Linear layer with Batch Normalization.
...
class Prenet(nn.Module):
    r"""Tacotron specific Prenet with an optional Batch Normalization.
    ...
    def __init__(
        ...
        in_features_list = [in_features] + out_features[:-1]

        if prenet_type == "bn":
            self.linear_layers = nn.ModuleList(
                [
                    LinearBN(in_size, out_size, bias=bias)
                    for (in_size, out_size) in zip(in_features_list, out_features)
                ],
            )
